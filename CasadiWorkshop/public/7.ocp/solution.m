...
figure
subplot(221)
spy(hessian(opti_MS.f,opti_MS.x))
title('MS Hessian')
% Only diagonal elements: no coupling between x or u,
% no coupling between control intervals

subplot(222)
spy(jacobian(opti_MS.g,opti_MS.x))
title('MS Jacobian')
...
subplot(223)
spy(hessian(opti_SS.f,opti_SS.x))
title('SS Hessian')
...
subplot(224)
spy(jacobian(opti_SS.g,opti_SS.x))
title('SS Jacobian')
...
full(sol_MS.value(jacobian(opti_MS.g,opti_MS.x),opti_MS.initial))
full(sol_SS.value(jacobian(opti_SS.g,opti_SS.x),opti_SS.initial))
...
%% 1.7
lag_MS = opti_MS.f+opti_MS.lam_g'*opti_MS.g;

G_MS = jacobian(opti_MS.g,opti_MS.x);
KKT_MS = [hessian(lag_MS,opti_MS.x) G_MS'; G_MS sparse(opti_MS.ng,opti_MS.ng)];

lag_SS = opti_SS.f+opti_SS.lam_g'*opti_SS.g;

G_SS = jacobian(opti_SS.g,opti_SS.x);
KKT_SS = [hessian(lag_SS,opti_SS.x) G_SS'; G_SS sparse(opti_SS.ng,opti_SS.ng)];

cond(sol_MS.value(KKT_MS,opti_MS.initial))
cond(sol_SS.value(KKT_SS,opti_SS.initial))
...
sol.value(U(:,1)) % Expected [-4.2894;-4.1930]
